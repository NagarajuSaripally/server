...
<div class="header" id="wrapper">
    ...
    <%-- <div id="page-content-title"><pwm:display key="Title_Login" displayIfMissing="true"/></div>
        ...
        <div class="wrapper">
    <form class="form-signin" action="<pwm:current-url/>" method="post" name="login" enctype="application/x-www-form-urlencoded" id="login" autocomplete="off">  
    <%@ include file="/WEB-INF/jsp/fragment/message.jsp" %>     
      <h2 class="form-signin-heading">Please login</h2>
      <%@ include file="/WEB-INF/jsp/fragment/ldap-selector.jsp" %>
      <input type="text" class="form-control" name="username" id="username"
						title="<pwm:display key="Field_Username"/>" 
						placeholder="<pwm:display key="Field_Username"/>" <pwm:autofocus/> required="required" />
      <input type="password" class="form-control" name="password" id="password"
						title="<pwm:display key="Field_Password"/>" placeholder="<pwm:display key="Field_Password"/>" required="required"/>      
      <%@ include file="/WEB-INF/jsp/fragment/captcha-embed.jsp"%>
      <button class="btn btn-lg btn-primary btn-block" type="submit" <pwm:autofocus/> name="button" id="submitBtn">Login</button>  
...
<%-- <div id="wrapper" class="container">
    ...
    <div id="centerbody" class="row">
        <div id="page-content-title"><pwm:display key="Title_Login" displayIfMissing="true"/></div>
        <p>
            <span class="panel-login-display-message"><pwm:display key="Display_Login"/></span>
        </p>
        <!--  Changes for custom Login -->
			<form action="<pwm:current-url/>" method="post" name="login" enctype="application/x-www-form-urlencoded" id="login" autocomplete="off" style="width:100%">
			<%@ include file="/WEB-INF/jsp/fragment/message.jsp" %>
				...
				<div class="form-group">
					<label for="password">Password</label> <input
						type="<pwm:value name="<%=PwmValue.passwordFieldType%>"/>" class="form-control" name="password" id="password"
						title="<pwm:display key="Field_Password"/>" placeholder="<pwm:display key="Field_Password"/>" required="required">
				</div>
				<%@ include file="/WEB-INF/jsp/fragment/captcha-embed.jsp"%>
				<div class="text-center">
                    ...
                    style="width: 50%;" 
                      <pwm:autofocus/> name="button" id="submitBtn">
            ...
            <pwm:if test="<%=PwmIfTest.showLoginOptions%>">
                <table class="noborder">
                    <pwm:if test="<%=PwmIfTest.forgottenPasswordEnabled%>">
                        <tr>
                            <td class="menubutton_key">
                                <a class="link animated fadeInUp delay-1s servicelink" id="Title_ForgottenPassword" href="<pwm:url addContext="true" url='<%=PwmServletDefinition.ForgottenPassword.servletUrl()%>'/>">
                                    <pwm:if test="<%=PwmIfTest.showIcons%>"></pwm:if>
                    ...
                    <pwm:if test="<%=PwmIfTest.forgottenUsernameEnabled%>">
                        <tr>
                            <td class="menubutton_key">
                                <a class="link animated fadeInUp delay-1s servicelink" id="forgotten-username" href="<pwm:url addContext="true" url='<%=PwmServletDefinition.ForgottenUsername.servletUrl()%>'/>">
                                    <pwm:if test="<%=PwmIfTest.showIcons%>"></pwm:if>
